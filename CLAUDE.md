...
- `src/components/` — primitives (`Screen`, `Button`, `Chip`, `Input`,
  ...
  `PermissionDialog` — all portal to `document.body`, animated via
  `usePresence`, marked `inert` + `aria-hidden` while exiting), `SmsScreen`,
  `MapView` (Denver image), `LocationPicker`, `DevHint`.
...
NotFound), Add a guest sheet, Edit cover sheet, Edit reservation, reminder
SMS variants. Inert-by-design controls: Copy, Add guest (+), Resend code,
Add preferences, Edit info, See full menu, the party-page menu rows.
