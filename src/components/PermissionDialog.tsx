...
export function PermissionDialog({ open, body, onAllow, onDeny }: Props) {
  ...
  return createPortal(
    <div className={`overlay overlay--center ${visible ? 'overlay--in' : ''}`} role="alertdialog" aria-modal="true" aria-label="Location permission" inert={!visible} aria-hidden={!visible}>
